...
/* Code Snippets used were reverse engineered in my 2.2 Decompiled Forked Repository */
class $modify(ProxyGameLevelManager, GameLevelManager){
	...
	void ProcessHttpRequest(gd::string endpoint, gd::string params, gd::string tag, GJHttpType httpType){
		...
		/* TODO: Fix this and Give it a public attribute of _requestTypeGJ and a reinterpret_cast on re-entry... */
    	// request->_requestTypeGJ = (int)httpType;
    	/* Send-Off */
    	ProxyOverride::CCProxyClient::getInstance()->send(request);
    	request->release();
	}
